...
class AuthService {
  ...
  async signIn(credentials: LoginCredentials): Promise<AuthUser> {
    ...
    window.localStorage.setItem("effectcord_user", JSON.stringify(this.currentUser))
  ...
  async signOut(): Promise<void> {
    ...
    window.localStorage.removeItem("effectcord_user")
    this.notify()
  }

  // Initialize auth state from localStorage
  init(): void {
    const stored = window.localStorage.getItem("effectcord_user")
    if (stored) {
      try {
